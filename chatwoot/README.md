...
Скрипт создаёт `.env` с секретом, поднимает стек, заводит аккаунт оператора и
Website-инбокс, подставляет токен в страницу сайта и печатает готовые ссылки:
...
Напишите что-нибудь в виджете на :8080 — сообщение появится у оператора на :3000.
Дальше можно пропустить ручные шаги ниже. `bootstrap.sh` — для локального демо;
для реального сервера смените пароли и `FRONTEND_URL`, уберите демо-аккаунт.

### Публичный URL через Cloudflare Tunnel

Если нужен адрес, который открывается с телефона и у коллег (а не только
`localhost`), на машине со свободным доступом в интернет:
...
Скрипт поднимает стек и два быстрых туннеля Cloudflare (аккаунт Cloudflare не
нужен), перенастраивает Chatwoot на публичный адрес и печатает две https-ссылки:

- **приложение оператора** — `https://<random>.trycloudflare.com`
- **страница сайта с виджетом** — `https://<random>.trycloudflare.com`

Откройте вторую ссылку, напишите в виджете — сообщение появится у оператора по
первой. Адреса случайные и живут, пока запущены туннели (`docker compose
-f docker-compose.yml -f docker-compose.tunnel.yml down` — остановить). Это
демо-режим; для постоянного адреса используйте свой домен (см. «Продакшен»).

> Из ограниченных сред (например, изолированных песочниц без доступа к сети
> Cloudflare) быстрый туннель не поднимется — запускайте на своей машине/VPS.
